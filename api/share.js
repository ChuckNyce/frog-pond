...
<head>
  ...
  <style>
    ...
    .share-action-btn.cta:hover { background: rgba(74,106,48,0.08); }
    .share-tagline {
      font-size: 11px;
      color: var(--dim);
      text-align: center;
...
<script>
  ...
  try {
    ...
    document.getElementById('share-tagline').classList.remove('hidden');
    ...
    document.getElementById('save-image').addEventListener('click', async function() {
      ...
      try {
        ...
        // ── Fixed top-right: ASCII frog, 40% opacity ──
        ctx.save();
        ctx.globalAlpha = 0.4;
        ctx.fillStyle = accentC;
        ctx.font = '16px ' + monoF; ctx.textBaseline = 'top';
        ['  @..@  ', ' (----) ', '( >__< )', ' ^^  ^^ '].forEach(function(line, i) {
        ...
        ctx.fillText(brand, (W - ctx.measureText(brand).width) / 2, 1010);
